...
SC.missions.M5 = (function () {
    ...
    self.filename = 'medium_metis_polar_landing.js';
    self.category = 'Medium missions';
    self.title = 'Metis polar landing';
    self.summary = 'Gently land on Metis\'s north pole, within 3m of landing pad and dV < 1m/s';
    ...
    self.init = function () {
        ...
        SC.gos.afterUpdate = function () {
            pad.pos = metis.pos.add(metis.up().multiply(-metis.radius));
        ...
        // rename PHOTO button to WIRE button
        SC.panel.buttons.photo.button.textContent = 'WIRE';

        self.event = function (aEvent) {
            // when user press PHOTO change metis' model to wireframe
            if (aEvent === 'photo') {
                metis.model = metis.model ? '' : 'sphere';
            }
        };
